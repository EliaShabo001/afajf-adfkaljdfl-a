...
import threading
from db import load_questions_by_batch, mark_link_used, is_link_used, is_link_expired, get_batch_metadata
import time
...
def start_test(chat_id, questions_list, batch_info):
    ...
    user_data[chat_id] = {
        ...
        "timer": None,
        "student_answers": []  # Store student's answers for review
    }
...
def finish_test(chat_id):
    ...
    # Send comprehensive review of all questions and answers
    send_quiz_review(chat_id, data["student_answers"])

def send_quiz_review(chat_id, student_answers):
    """Send a comprehensive review of all questions and student's answers"""
    if not student_answers:
        return

    # Send review header
    review_header = (
        f"📋 مراجعة شاملة للاختبار\n"
        f"═══════════════════════\n\n"
        f"📝 فيما يلي جميع الأسئلة مع إجاباتك والإجابات الصحيحة:\n\n"
    )

    student_bot.send_message(chat_id, review_header, protect_content=True)

    # Send each question with detailed review
    for i, answer_data in enumerate(student_answers):
        question_review = format_question_review(answer_data)

        try:
            student_bot.send_message(chat_id, question_review, protect_content=True)

            # Add small delay between messages to avoid rate limiting
            if i < len(student_answers) - 1:  # Don't delay after last message
                time.sleep(0.5)

        except Exception as e:
            print(f"Error sending question review {i+1}: {e}")

    # Send review footer
    review_footer = (
        f"═══════════════════════\n"
        f"✅ انتهت المراجعة الشاملة\n\n"
        f"💡 استخدم هذه المراجعة لفهم الأخطاء وتحسين أدائك\n"
        f"📚 راجع التفسيرات لفهم الإجابات الصحيحة\n"
        f"🚫 ممنوع مشاركة محتوى الاختبار"
    )

    student_bot.send_message(chat_id, review_footer, protect_content=True)

def format_question_review(answer_data):
    """Format a single question review with student's answer and correct answer"""
...
def handle_answer(call):
    ...
    # Store student's answer for review
    answer_record = {
        ...
        "is_correct": chosen_text == correct_answer,
        "description": question.get("description", "")
    }
    user_data[chat_id]["student_answers"].append(answer_record)
    ...
    if chosen_text == correct_answer:
        ...
        feedback_msg = "✅ إجابة صحيحة! أحسنت."
        ...
        student_bot.send_message(chat_id, feedback_msg, protect_content=True)
    else:
        user_data[chat_id]["incorrect"] += 1
        student_bot.answer_callback_query(call.id, "❌ إجابة خاطئة")

        # Prepare feedback message
        feedback_msg = f"❌ إجابة خاطئة.\n💡 الإجابة الصحيحة هي: {correct_answer}"
