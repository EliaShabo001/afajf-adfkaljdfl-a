...
def handle_insert_steps(message):
    ...
    if state["step"] == "ask_subject":
        ...
        # Show the question and correct answer, then ask for description
        bot.send_message(chat_id,
            f"✅ الإجابة الصحيحة: {correct_text}\n\n"
            f"📝 الآن أرسل وصف أو تفسير لماذا هذه الإجابة صحيحة:\n"
            f"(يمكنك كتابة تفسير مفصل يساعد الطلاب على فهم السبب)\n\n"
            f"💡 أو اكتب 'بدون وصف' إذا كنت لا تريد إضافة تفسير")

    elif state["step"] == "ask_description":
        description = message.text.strip()

        # Allow skipping description
        if description.lower() in ['بدون وصف', 'skip', 'no description', '']:
            description = ""
        ...
        if state["current"] == state["total"]:
            try:
                ...
                bot.send_message(chat_id,
                    ...
                    f"📚 الموضوع: {state['subject']}\n"
                    f"⏱️ مدة الاختبار: {duration} دقيقة\n"
                ...
                bot.send_message(chat_id, f"❌ حدث خطأ أثناء إنشاء الرابط: {str(e)}\nيرجى المحاولة مرة أخرى.")
                print(f"Error creating quiz link: {e}")
...
def handle_active_question_link_again(message):
    ...
    for batch in batches:
        ...
        # Determine status with expiry check
        if batch.get('expired', False):
            status_emoji = "⏰ منتهي الصلاحية"
        elif batch['used']:
            status_emoji = "❌ مستخدم"
        else:
            status_emoji = "✅ نشط"
        ...
        if batch.get('expires_at'):
            ...
            try:
                ...
                msg += f"⏰ ينتهي في: {expires_at.strftime('%Y-%m-%d %H:%M')}\n"
            except:
                pass
    ...
    for batch in active_batches:
        # Show used or expired batches for reactivation
        if batch['used'] or batch.get('expired', False):
