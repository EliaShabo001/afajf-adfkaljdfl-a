    ...
    def start_teacher_bot_thread(self):
        ...
        def run_teacher():
            try:
                ...
                # Import telebot and create bot instance directly to avoid proxy issues
                import telebot

                # Get token from environment or use default
                teacher_token = os.getenv('TEACHER_BOT_TOKEN', '7372608784:AAGipEjvwZrxlibKGRtcoQFGYEYJWb9amBE')

                # Create bot instance without proxy
                self.teacher_bot = telebot.TeleBot(teacher_token)

                # Import the handlers from TelegramBot module
                try:
                    import TelegramBot
                    # Copy handlers from the original bot to our new instance
                    self.teacher_bot.message_handlers = TelegramBot.bot.message_handlers
                    self.teacher_bot.callback_query_handlers = TelegramBot.bot.callback_query_handlers
                    self.log("✅ Teacher Bot handlers loaded")
                except Exception as handler_error:
                    self.log(f"⚠️ Could not load handlers: {handler_error}")
                    # Fallback: use the original bot
                ...
                self.teacher_bot.remove_webhook()
                self.log("✅ Teacher Bot started successfully")
                self.teacher_bot.polling(none_stop=True, interval=1, timeout=20)

            except Exception as e:
                self.log(f"❌ Teacher Bot error: {e}")
                import traceback
                self.log(f"❌ Teacher Bot traceback: {traceback.format_exc()}")
                self.teacher_running = False
    ...
    def start_student_bot_thread(self):
        ...
        def run_student():
            try:
                ...
                # Create bot instance without proxy
                self.student_bot = telebot.TeleBot(student_token)

                # Import the handlers from StudentBot module
                try:
                    import StudentBot
                    # Copy handlers from the original bot to our new instance
                    self.student_bot.message_handlers = StudentBot.student_bot.message_handlers
                    self.student_bot.callback_query_handlers = StudentBot.student_bot.callback_query_handlers
                    self.log("✅ Student Bot handlers loaded")
                except Exception as handler_error:
                    self.log(f"⚠️ Could not load handlers: {handler_error}")
                    # Fallback: use the original bot
                ...
                self.student_bot.polling(none_stop=True, interval=1, timeout=20)

            except Exception as e:
                self.log(f"❌ Student Bot error: {e}")
                import traceback
                self.log(f"❌ Student Bot traceback: {traceback.format_exc()}")
                self.student_running = False
